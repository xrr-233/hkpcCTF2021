...
class Key:
    def __init__(self, p, q):
        self.p = p
        self.q = q
        self.n = p*q # 
        self.e = E
        phi = (p-1) * (q-1)
...
class User:
    ...
    def send(self, m):
        ...
        m = int(m, 16)
        print(hex(self.key.encrypt(m))) # RSA加密
...
def main():
    ...
    flag = pad(flag.encode(), 16) # 补全至16的倍数

    master_secret = os.urandom(32) # 16*16^32
    admin = User(master_secret)

    for _ in range(CALLS): # 允许17次测试
